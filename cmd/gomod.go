...
func GetWailsVersion() (*semver.Version, error) {
	...
	// Find wails version
	versionRegexp := regexp.MustCompile(`.*github.com/cllsm/wails-win7.*(v\d+.\d+.\d+(?:-pre\d+)?)`)
...
func UpdateGoModVersion() error {
	...
	requireLine := "-require=github.com/cllsm/wails-win7@v" + currentVersionString

	// Issue: go mod edit -require=github.com/cllsm/wails-win7@1.0.2-pre5
